const SOURCE_NAMES: Record<string, string> = {
  ...
  "vdekje.txt": "Gjendja Civile · e-albania.al",
...
export function friendlySource(rawSource: string | null | undefined): string | null {
  ...
  const withExtension = `${basename}.txt`;
  if (SOURCE_NAMES[withExtension]) return SOURCE_NAMES[withExtension]!;
  const underscoredWithExtension = `${basename.replace(/\s+/g, "_")}.txt`;
  if (SOURCE_NAMES[underscoredWithExtension]) return SOURCE_NAMES[underscoredWithExtension]!;
